			...
			client: fake.NewFakeClientBuilder().
				WithInterceptorFuncs(interceptor.Funcs{
					SlurmV0043GetDiagWithResponse: func(ctx context.Context, reqEditors ...api.RequestEditorFn) (*api.SlurmV0043GetDiagResponse, error) {
						return &api.SlurmV0043GetDiagResponse{
				...
				WithInterceptorFuncs(interceptor.Funcs{
					SlurmV0043GetDiagWithResponse: func(ctx context.Context, reqEditors ...api.RequestEditorFn) (*api.SlurmV0043GetDiagResponse, error) {
						return &api.SlurmV0043GetDiagResponse{
							...
							JSONDefault: &api.V0043OpenapiDiagResp{
	...
	tests := []struct {
	...
	}{
		{
			...
			client: fake.NewFakeClientBuilder().
				...
				Build(),
			want:    &types.V0043DiagList{Items: []types.V0043Diag{{}}},
			wantErr: false,
		},
		{
			name: "HTTP Error",
