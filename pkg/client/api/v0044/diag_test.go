			...
			client: fake.NewFakeClientBuilder().
				WithInterceptorFuncs(interceptor.Funcs{
					SlurmV0044GetDiagWithResponse: func(ctx context.Context, reqEditors ...api.RequestEditorFn) (*api.SlurmV0044GetDiagResponse, error) {
						return &api.SlurmV0044GetDiagResponse{
				...
				WithInterceptorFuncs(interceptor.Funcs{
					SlurmV0044GetDiagWithResponse: func(ctx context.Context, reqEditors ...api.RequestEditorFn) (*api.SlurmV0044GetDiagResponse, error) {
						return &api.SlurmV0044GetDiagResponse{
							...
							JSONDefault: &api.V0044OpenapiDiagResp{
	...
	tests := []struct {
	...
	}{
		{
			...
			client: fake.NewFakeClientBuilder().
				...
				Build(),
			want:    &types.V0044DiagList{Items: []types.V0044Diag{{}}},
			wantErr: false,
		},
		{
			name: "HTTP Error",
